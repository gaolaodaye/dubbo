...
import com.mryx.wechat.WeChatSend;
	...
	private ExchangeHandler requestHandler = new ExchangeHandlerAdapter() {
		...
		public void disconnected(Channel channel) throws RemotingException {
			if (logger.isInfoEnabled()) {
				...
				WeChatSend.sendText("5", "gaofeng", "服务器[" + channel.getRemoteAddress() + "]挂了,检查一下吧:" + channel.getUrl());
			}
	...
	private ExchangeClient getSharedClient(URL url) {
		...
		if (client != null) {
			if (!client.isClosed()) {
				...
				// logger.warn(new IllegalStateException("client is closed,but
				// stay in clientmap .client :"+ client));
